...
function firstVisibleDay(date, localizer) {
  var firstOfMonth = dates.startOf(date, 'month') // Make sure the previous two weeks are always visible

  var start = dates.startOf(firstOfMonth, 'week', localizer.startOfWeek())
  var diffInDays = dateDiffInDays(firstOfMonth, date)

  if (diffInDays < 15) {
    var day = dates.subtract(date, 14, 'day')
...
function lastVisibleDay(date, localizer) {
  ...
  var endOfMonth = dates.add(dates.endOf(date, 'month'), 1, 'week') // Show an extra week
